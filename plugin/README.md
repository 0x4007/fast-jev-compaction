...
probability reaches `dropThreshold` and the protection probability stays below
`protectThreshold`. Tool-call messages are evaluated together with their
...
to Claude Code's built-in compaction. Both the fallback and the success log
line include the estimated reduction and a count of decisions by reason
(`pinned`, `protected`, `below_threshold`, `dropped`). The `turn.complete` hook requests
