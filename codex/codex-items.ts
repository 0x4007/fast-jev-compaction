/**
 * Pure mapping between Codex Responses API input items and the
 * fast-jev-compaction library `Message` shape, plus the deterministic renderer
 * that turns Jev's keep/drop decisions back into one text summary.
 *
 * No I/O, no Deno/Node APIs: importable from the Deno proxy and from vitest.
 * The library is imported from the built `dist/` output because Deno cannot
 * resolve the library's `./x.js` specifiers back to `src/*.ts`.
 */
import { truncatedResultText } from '../dist/index.js';
import type {
  CallDecision,
  CompactResult,
  Message,
...
/** First line of every rendered summary; never the library's SUMMARY_PREFIX. */
export const SUMMARY_MARKER = '# fast-jev-compaction memory summary';

/**
 * Body of Codex's built-in local-compaction prompt
 * (`codex-rs/prompts/templates/compact/prompt.md`, codex 0.154.0). Used only to
 * recognise the prompt Codex appends as the final input item, never to detect
 * whether a request is a compaction request (that is header-only).
 */
export const COMPACTION_PROMPT_MARKERS = [
...
export type TranscriptEntry = TextEntry | CallEntry | ResultEntry;
...
export function parseToolArguments(argumentsText: string): UnknownRecord {
 ...
 */
export function opaquePlaceholder(itemType: string, item: UnknownRecord, limit = 2000): {
  text: string;
  opaque: boolean;
} {
  let json = '';
...
function messageEntry(item: UnknownRecord): TextEntry {
  ...
  const normalized: TextEntry['role'] = role === 'assistant'
    ? 'assistant'
...
function callEntry(item: UnknownRecord): CallEntry {
  ...
  const rawInput = functionCall
    ? parseToolArguments(str(item.arguments))
    : (() => {
...
export function renderSummary(transcript: CodexTranscript, options: RenderOptions): string {
  ...
  for (const entry of transcript.entries) {
    ...
    if (entry.kind === 'call') {
      ...
      out.push(
        `[tool call ${entry.callId || '(no call id)'}] ${entry.tool || '(unnamed tool)'} — ${
          decision ? decision.action : 'no decision (kept)'
        }`,
    ...
    if (!decision) {
      out.push(
        `[tool result ${entry.callId || '(no call id)'} — no matching call in this transcript]`,
        entry.text,
        '',
      );
      continue;
  ...
  out.push(
    '[fast-jev-compaction stats] ' +
      [
        ...
        stats ? `calls=${stats.resultsDropped + stats.callsDropped + stats.kept + stats.pinned}` : 'calls=0',
        stats ? `kept=${stats.kept}` : 'kept=0',
        stats ? `results_dropped=${stats.resultsDropped}` : 'results_dropped=0',
        stats ? `calls_dropped=${stats.callsDropped}` : 'calls_dropped=0',
        stats ? `pinned=${stats.pinned}` : 'pinned=0',
        stats ? `chars_before=${stats.charsBefore}` : 'chars_before=0',
        stats ? `chars_after=${stats.charsAfter}` : 'chars_after=0',
        stats ? `requests=${stats.requests}` : 'requests=0',
        stats ? `stage=${stats.stateStage || 'none'}` : 'stage=none',
      ].join(' '),
  );
  return out.join('\n');
}
...
export function isResponsesCompaction(metadata: TurnMetadata | null): boolean {
  return metadata?.request_kind === 'compaction' &&
    record(metadata.compaction)?.implementation === 'responses';
}

export function isCompactionKind(metadata: TurnMetadata | null): boolean {
  return metadata?.request_kind === 'compaction';
}
