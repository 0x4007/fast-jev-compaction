...
`5c583fe89bbd3ab4dc9a05768299f94e52fe8452` (the `vendor/codex` gitlink). At the
time of writing the worktree carries uncommitted changes on top of that pin:
`core/src/working_set.rs` (new), `core/src/codex.rs`,
`core/src/rollout/list.rs`, `core/src/lib.rs`, `protocol/src/config_types.rs`,
`tui/src/history_cell.rs`, `core/Cargo.toml`, `Cargo.lock`. The
`rollout/list.rs` change is the product fix for the resume sidecar collision
(§6) and is still pending compilation/acceptance.
...
There is **no recorded final green for the actual pinned-client suite**. The
harness previously installed a test-only
`<CODEX_HOME>/sessions/.ignore` rule so the client's fuzzy `resume <uuid>`
lookup skipped the same-UUID sidecar; that workaround has now been removed
because it masked the product defect instead of testing it. The surviving
recorded run used the workaround, so after the removal the resume-dependent
tests are expected to fail until the in-progress `rollout/list.rs` fix is
compiled and re-run. The final accepted result must be amended here only after
that receipt; do not read this document as a final-green claim.
...
- **Uncommitted fork state.** The fork changes are not committed/pushed at the
  time of writing; the parent `vendor/codex` gitlink still points at the base
  pin. Advancing it is a separate, later step.
